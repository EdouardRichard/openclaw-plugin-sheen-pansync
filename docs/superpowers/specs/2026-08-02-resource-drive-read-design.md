...
6. 提供带真实、脱敏测试截图的中英文 `README.md`，让用户能够快速完成安装、配置、首次上传和首次读取。
...
根目录 `README.md` 是发布包中的主要上手入口，必须同时提供完整中文和完整英文内容，而不是只翻译标题或示例。文档顶部提供中文、English 锚点，使用户可以直接跳转到自己的语言。

中英文两部分保持相同的信息顺序：

1. 插件价值与能力边界。
2. OpenClaw、Node.js 和阿里云盘账号等前置要求。
3. 可复制执行的安装与启用命令。
4. `openclaw pan-sync configure` 授权配置流程。
5. 状态确认与常见恢复方式。
6. 第一次上传到资源盘。
7. 第一次列目录、搜索、下载并让 OpenClaw 读取文件。
8. 超过 100 MiB 文件的二次确认行为。
9. 中英文自然语言示例与不会触发 Tool 的反例。
10. 安全说明、已知限制与卸载或重新配置入口。

README 必须明确：所有文件操作只针对阿里云盘资源盘；下载默认写入当前工作区根目录且不会覆盖同名文件；插件下载后由 OpenClaw 的常规文件能力读取正文。

测试过程中截取并保存真实产品截图，建议放在 `docs/images/readme/`：
...
- 中文或英文上传请求及安全结果；
- 网盘列出或搜索结果；
- 下载到工作区并由 OpenClaw 读取后的结果。

截图必须来自实际安装或验收流程，不使用绘制的示意图替代。截图前使用专用测试文件和非敏感演示名称；截图中不得出现 Refresh Token、Access Token、一次性 URL fragment、动态端口、真实账号标识、文件标识、本地绝对路径、主机名、用户名、原始 Shell 或服务日志。若原始界面包含这些内容，应重新组织测试数据和视图后重拍；只有无法重拍的非敏感区域才允许裁切，不能依靠模糊遮挡来掩盖秘密。

每张图片使用中英文可理解的替代文本和相邻说明。README 中的相对图片链接必须存在，图片必须随 npm 包发布，并通过包内容测试验证。图片尺寸和压缩应兼顾清晰度与包体积，不加入与快速上手无关的装饰图。
...
### 11.4 Tool、技能、README 与打包

- Tool Schema 拒绝无效参数组合，结果只包含批准字段。
- 清单、插件注册、实际安装包和 OpenClaw registry 均包含三个 Tool。
- 双语技能包含中英文正例、同步方向规则、多结果选择、大文件确认和讨论性负例。
- README 的中英文锚点、章节顺序、命令、图片链接和关键产品声明都有自动化检查。
- README 引用的每张图片存在、可解码、尺寸合理，并随实际 npm 包发布。
- 发布包只包含批准的技能、README 图片与运行时代码，不包含测试、状态、凭证或构建临时文件。
...
9. 在实际安装、配置、上传、查询和读取流程中拍摄 README 所需截图。
10. 对照截图逐张检查秘密、动态端口、真实身份、本地绝对路径和原始日志泄漏。
11. 分别按 README 中文部分和英文部分从头执行一次快速上手，确认命令可复制、步骤无跳跃、图片与当前界面一致。
...
5. 扩展双语技能、清单和包内容测试。
6. 通过实际安装与功能验收取得脱敏截图，并编写完整中英文 README 快速上手。
7. 验证 README 链接、图片和发布包内容，再运行完整自动化门禁。
8. 在具备专用账号时执行真实资源盘验收并单独记录结果。
